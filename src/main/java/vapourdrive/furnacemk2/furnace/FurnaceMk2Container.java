...
//    }
    ...
    public FurnaceMk2Container(int windowId, Level world, BlockPos pos, Inventory inv, Player player, FurnaceData furnaceData) {
        ...
        if (tileEntity != null) {
            tileEntity.getCapability(ForgeCapabilities.ITEM_HANDLER, null).ifPresent(h -> {
                addSlot(new SlotCore(h, 0, 12, 17, Registration.INSULATION_CORE_ITEM.get()));
                addSlot(new SlotCore(h, 1, 12, 35, Registration.THERMAL_CORE_ITEM.get()));
                addSlot(new SlotCore(h, 2, 12, 53, Registration.EXPERIENCE_CORE_ITEM.get()));
                addSlot(new SlotFuel(h, 3, 49, 53));
                addSlot(new SlotIngredient(h, 4, 49, 17, this.world));
                addSlot(new SlotOutput(h, 5, 94, 17));
                addSlot(new SlotOutput(h, 6, 112, 17));
                addSlot(new SlotOutput(h, 7, 130, 17));
                addSlot(new SlotOutput(h, 8, 148, 17));
    ...
    public float getCookProgress() {
        ...
        return (float) i / (float) this.furnaceData.get(1);
    }
